  ...
  if(length(aspectMatch) > 0){  
    ...
    if(length(matchId) > 0){ 
      ...
      for(j in 1:indLen){
        ...
        }else if(recPost == "N" & recDag == "N"){recDag <- "N"}else if(recPost == "P" & recDag == "P"){
            recDag <- "P"
            Pcou <- Pcou + 1
            tempPos <- match(intersect(cleantxtSplit[tempInd],postDic),postDic ) # Adding the index of positive in a             temporary container
            positive <- c(positive,tempPos) # collecting the positive counts 
        ...
        if(aspect == "food"){
          ...
          if(asCheck > 0){
            ...
            entList <- list(ent = entity,pola = polarity,pos = positive,neg =negative,comp = competitor,polc =     polChanger,pcpos = pcPos,pcneg = pcNeg)
          ...
          if(asCheck > 0){
            ...
            # Joining together all the metadata
            entList <- entList <- list(ent = entity,pola = polarity,pos = positive,neg =negative,comp = competitor,polc = polChanger,pcpos = pcPos,pcneg = pcNeg)
...
```{r}
testList <- recScanner(cleantxtSplit,"price")$dagStore 
```
